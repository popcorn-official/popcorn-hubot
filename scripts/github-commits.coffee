  ...
  robot.router.post "/hubot/gh-commits", (req, res) ->
    ...
    try
      ...
      if payload.commits.length > 0
        ...
        robot.send user, "Got #{payload.commits.length} new #{commitWord} from #{payload.commits[0].author.name} on #{payload.repository.name}  (Branch: " + payload.ref.replace("refs/heads/", "") + ")"
